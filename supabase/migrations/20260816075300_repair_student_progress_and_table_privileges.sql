...
as $$
  ...
  left join public.schoolcoin_requests q
    on q.activity_id = a.id
   and q.student_id = i.student_id
  where a.active = true
...
begin
  ...
  select requires_evidence, max_per_month
    into v_requires_evidence, v_max_per_month
  from public.schoolcoin_activities
  ...
  if v_pending_exists then
    raise exception 'Bu faoliyat uchun kutayotgan so‘rovingiz allaqachon mavjud';
  end if;
  ...
  if v_max_per_month is not null then
    ...
    if v_month_count >= v_max_per_month then
      raise exception 'Bu faoliyat uchun oylik limit tugagan (% ta)', v_max_per_month;
    end if;
  end if;
  ...
  if v_requires_evidence then
    ...
    if not exists (
      ...
      where o.bucket_id = 'schoolcoin-evidence'
        and o.name = v_evidence_path
