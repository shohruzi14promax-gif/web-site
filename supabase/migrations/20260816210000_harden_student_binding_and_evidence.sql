...
begin
  ...
  if v_existing_auth is not null and v_existing_auth <> v_auth_user_id then
    raise exception 'Bu student allaqachon boshqa Auth sessiyasiga bog‘langan';
  end if;
  if exists (select 1 from public.schoolcoin_student_auth_bindings where auth_user_id = v_auth_user_id and student_id <> v_student.id) then
    raise exception 'Bu Auth sessiyasi boshqa studentga bog‘langan';
  end if;
...
begin
  ...
  if v_requires_evidence then
    ...
    if not exists (select 1 from storage.objects o where o.bucket_id='schoolcoin-evidence' and o.name=v_evidence_path and o.owner_id=auth.uid()) then
