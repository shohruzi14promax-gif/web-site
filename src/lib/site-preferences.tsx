...
export function SitePreferencesProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    localStorage.setItem('site-locale', locale); document.documentElement.lang = locale;
    const url = new URL(window.location.href); url.searchParams.set('lang', locale); window.history.replaceState({}, '', url);
    const current = metadata[locale]; document.title = current.title;
  ...
  useEffect(() => { localStorage.setItem('site-theme', theme); document.documentElement.dataset.theme = theme; document.querySelector('meta[name="theme-color"]')?.setAttribute('content', theme === 'dark' ? '#0f172a' : '#f8fafc'); }, [theme]);
