...
export async function getSiteData<T>(key: SiteDataKey, fallback: T): Promise<T> {
  ...
  } catch (error) { console.error(`Supabase getSiteData(${key}) failed:`, error); return fallback; }
}
...
export async function signInAdmin(email: string, password: string) {
  ...
  if (error) throw error;
  if (data.user?.app_metadata?.role !== 'admin') { await supabase.auth.signOut({ scope: 'local' }); throw new Error('Bu akkaunt admin huquqiga ega emas.'); }
  return data;
}
...
export async function ensureStudentAuthSession() {
  ...
  if (sessionData.session) {
    if (!sessionData.session.user.is_anonymous) throw new Error('Avval joriy admin sessiyasidan chiqing.');
...
export async function bindStudent(code: string, pin: string) {
  const { data, error } = await supabase.rpc('schoolcoin_bind_student', { p_code: code.trim().toUpperCase(), p_pin: pin });
  if (error) throw error;
