...
export default function App() {
  ...
  useEffect(() => {
    ...
    loadNotifications();
    if (supabaseConfigured) void Promise.all([syncNotificationKey('announcements'), syncNotificationKey('birthdays')]).catch(() => undefined);
    const channel = supabaseConfigured ? supabase.channel('site-notifications-live').on('postgres_changes', { event: '*', schema: 'public', table: 'site_data' }, payload => {
  ...
  return (
    <div className="site-shell min-h-screen relative text-slate-900 selection:bg-[#0071e3] selection:text-white" style={{ backgroundImage: `linear-gradient(to bottom, rgba(248,250,252,.72), rgba(248,250,250,.54)), url('/images/maktab-bg.JPG')`, backgroundSize: 'cover', backgroundPosition: 'center top', backgroundAttachment: 'scroll', backgroundRepeat: 'no-repeat' }}>
      ...
      <button type="button" onClick={() => setIsSchoolCoinOpen(true)} className="fixed bottom-24 right-4 z-40 flex items-center gap-2 rounded-full bg-amber-500 px-4 py-3 text-white shadow-2xl transition-transform duration-200 hover:scale-105 active:scale-95 sm:right-6 sm:px-5" title="SchoolCoin" aria-label="SchoolCoin"><Coins className="h-5 w-5" /><span className="hidden text-sm font-bold sm:inline">SchoolCoin</span></button>
      <button type="button" onClick={() => setIsAdminOpen(true)} className="fixed bottom-5 right-4 z-50 flex items-center gap-2 rounded-full border border-white/20 bg-slate-900/90 px-4 py-3 text-white shadow-2xl backdrop-blur-xl transition-transform duration-200 hover:scale-105 active:scale-95 sm:right-6 sm:px-5" title="Admin Panel" aria-label="Admin Panel"><Settings className="h-5 w-5" /><span className="hidden text-sm font-semibold sm:inline">Admin Panel</span></button>

      {totalNotifications > 0 && <button type="button" onClick={() => setShowNotifModal(true)} className="fixed bottom-5 left-4 z-40 flex items-center gap-2.5 rounded-full bg-[#0071e3] px-4 py-3 text-white shadow-xl transition-transform duration-200 hover:scale-105 active:scale-95 sm:left-6" aria-label={`Yangi bildirishnomalar: ${totalNotifications}`}><Bell className="h-5 w-5" /><span className="rounded-full bg-white px-2 py-0.5 text-xs font-bold text-[#0071e3]">{totalNotifications}</span><span className="hidden text-sm font-medium sm:inline">E'lonlar va Tadbirlar</span></button>}

      {showNotifModal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-md motion-reduce:backdrop-blur-none" role="presentation" onClick={() => setShowNotifModal(false)}><div className="relative w-full max-w-lg overflow-hidden rounded-[28px] bg-white/95 p-5 shadow-2xl sm:p-6" role="dialog" aria-modal="true" aria-labelledby="notification-title" onClick={event => event.stopPropagation()}><div className="mb-4 flex items-center justify-between border-b pb-3"><h3 id="notification-title" className="flex items-center gap-2 text-base font-bold"><Megaphone className="h-5 w-5 text-[#0071e3]" />Yangi bildirishnomalar</h3><button type="button" onClick={() => setShowNotifModal(false)} aria-label="Yopish" className="rounded-full p-2 hover:bg-slate-100"><X className="h-5 w-5" /></button></div><div className="max-h-[60vh] space-y-3 overflow-y-auto">{birthdays.map(b => <div key={`b-${b.id}`} className="flex gap-3 rounded-2xl border border-pink-100 bg-pink-50 p-4"><Cake className="h-5 w-5 text-pink-600" /><div><h4 className="text-sm font-semibold">Tug'ilgan kun! 🎂</h4><p className="mt-1 text-xs"><strong>{b.name}</strong> ({b.class}) ning bugun tug'ilgan kuni!</p></div></div>)}{announcements.map(a => <div key={`a-${a.id}`} className="flex gap-3 rounded-2xl border border-blue-100 bg-blue-50 p-4"><Megaphone className="h-5 w-5 text-[#0071e3]" /><div><h4 className="text-sm font-semibold">{a.title}</h4><p className="mt-1 text-xs">{a.content || a.description || a.message}</p><span className="text-[10px] text-slate-400">{a.date}</span></div></div>)}</div></div></div>}
