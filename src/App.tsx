...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (supabaseConfigured) {
      channel = supabase.channel('site-data-live')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'site_data' }, payload => {
          ...
          if (key && siteKeys.includes(key as typeof siteKeys[number])) void syncKey(key as typeof siteKeys[number]); else void syncCloud();
        })
        .subscribe();
    }
    ...
    return () => { alive = false; if (channel) void supabase.removeChannel(channel); window.removeEventListener('admin_data_updated', loadLocal); window.removeEventListener('storage', loadLocal); };
  }, []);
  ...
  return <div className="min-h-screen text-slate-900 selection:bg-[#0071e3] selection:text-white relative" style={{backgroundImage:`linear-gradient(to bottom, rgba(248,250,252,.65), rgba(248,250,252,.45)), url('/images/maktab-bg.JPG')`,backgroundSize:'cover',backgroundPosition:'center',backgroundAttachment:'fixed',backgroundRepeat:'no-repeat'}}>
    ...
    <button onClick={()=>setIsSchoolCoinOpen(true)} className="fixed bottom-24 right-6 z-50 flex items-center gap-2 rounded-full bg-amber-500 px-5 py-3 text-white shadow-2xl transition-all duration-300 hover:bg-amber-600 hover:scale-105 active:scale-95 cursor-pointer" title="SchoolCoin"><Coins className="h-5 w-5"/><span className="text-sm font-bold">SchoolCoin</span></button>
