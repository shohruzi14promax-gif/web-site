...
const resultIcons = [Trophy, GraduationCap, Target];

export default function Results() {
  const resultStats = stats.slice(3);
    ...
    <section id="results" className="mx-auto w-full max-w-7xl px-5 py-10 sm:px-8 sm:py-16 md:px-12 lg:px-16">
      <div className="rounded-[32px] border border-slate-200/70 bg-white/80 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)] backdrop-blur-xl sm:p-10 md:p-12">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-4 text-3xl font-black tracking-tight text-slate-900 sm:text-4xl md:text-5xl">Natijalarimiz gapiradi.</h2>
          <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-slate-600 sm:text-base">
            Maktabimizning yutuqlari va bitiruvchilar natijalarini bir joyda ko‘ring.
          </p>
        </div>

        <div className="mt-10 grid gap-3 sm:grid-cols-3">
          {resultStats.map((stat, index) => {
            const Icon = resultIcons[index] ?? Award;
            return (
              <article key={`${stat.label}-${index}`} className="rounded-3xl border border-slate-200/70 bg-slate-50/80 p-5 transition-transform duration-300 hover:-translate-y-1 hover:bg-white hover:shadow-lg motion-reduce:transition-none">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-3xl font-black tracking-tight text-slate-900 sm:text-4xl">{stat.value.toLocaleString()}{stat.suffix}</p>
                    <p className="mt-2 text-sm font-semibold leading-6 text-slate-600">{stat.label}</p>
                  </div>
                  <span className="rounded-2xl bg-white p-3 text-[#0071e3] shadow-sm" aria-hidden="true"><Icon className="h-5 w-5" /></span>
                </div>
              </article>
            );
          })}
