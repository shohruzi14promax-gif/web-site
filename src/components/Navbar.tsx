...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-3 sm:px-5">
      <nav className={`mx-auto flex max-w-7xl items-center justify-between transition-all duration-300 ${scrolled ? 'glass-surface mt-3 rounded-2xl px-4 py-2.5 sm:px-5' : 'px-2 py-4 sm:px-3'}`}>
        ...
        <div className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-0.5 lg:flex">
          {menuLinks.map(link => (
            <button key={link.href} type="button" onClick={() => handleNavClick(link.href)} className="relative rounded-full px-3.5 py-2 text-[13px] font-medium text-slate-600 transition-all duration-200 hover:bg-white/55 hover:text-[#0b1424] hover:backdrop-blur-md focus-visible:bg-white/60">
              {link.label}
            </button>
          ))}
        </div>
        ...
        <div ref={menuRef} className="relative lg:hidden">
          ...
          <div id="mobile-navigation" className={`glass-surface absolute right-0 top-[52px] w-[min(90vw,360px)] overflow-hidden rounded-[24px] transition-all duration-300 ${menuOpen ? 'visible translate-y-0 scale-100 opacity-100' : 'invisible -translate-y-2 scale-[.98] opacity-0'}`}>
            <div className="p-2.5">
              {menuLinks.map(link => (
                <button key={link.href} type="button" onClick={() => handleNavClick(link.href)} className="flex min-h-11 w-full items-center rounded-2xl px-4 py-3 text-left text-[15px] font-medium text-slate-700 transition-colors hover:bg-white/55 hover:text-[#0071e3]">
                  {link.label}
                </button>
              ))}
            </div>
