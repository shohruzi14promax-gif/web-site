...
const menuLinks = [navLinks[0], navLinks[1], navLinks[2], { href: '#school-life', label: 'Maktab hayoti' }, ...navLinks.slice(3)];
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 px-0 transition-[padding] duration-300 ${scrolled ? 'sm:px-5' : ''}`}>
      <nav className={`mx-auto flex max-w-7xl items-center justify-between px-5 sm:px-8 md:px-12 lg:px-16 transition-[margin,padding,border-radius,background-color,box-shadow] duration-300 ${scrolled ? 'mt-3 rounded-2xl border border-white/70 bg-white/75 py-2.5 shadow-lg shadow-slate-900/5 backdrop-blur-2xl' : 'bg-transparent py-3'}`}>
        <button type="button" onClick={() => handleNavClick('#hero')} className="group flex items-center gap-2.5 rounded-xl transition-transform duration-200 hover:scale-[1.02] active:scale-95" aria-label="Bosh sahifaga o'tish">
        ...
        <div className="hidden items-center gap-1 lg:flex">
          {menuLinks.map(link => <button type="button" key={link.href} onClick={() => handleNavClick(link.href)} className="rounded-full px-3.5 py-2 text-sm font-medium text-[#1d1d1f] transition-[background-color,color,transform] duration-200 hover:bg-white/80 hover:text-[#0071e3] hover:scale-[1.02] active:scale-95">{link.label}</button>)}
        </div>
        ...
        <div ref={menuRef} className="relative lg:hidden">
          ...
          <div id="mobile-navigation" className={`absolute right-0 top-12 w-[min(90vw,360px)] overflow-hidden rounded-3xl border border-white/70 bg-white/95 shadow-2xl shadow-slate-900/10 backdrop-blur-2xl transition-[opacity,transform,visibility] duration-200 ${menuOpen ? 'visible translate-y-0 opacity-100' : 'invisible -translate-y-2 opacity-0'}`}>
            <div className="flex flex-col p-3">
              {menuLinks.map(link => <button type="button" key={link.href} onClick={() => handleNavClick(link.href)} className="rounded-2xl px-4 py-3.5 text-left text-base font-medium text-[#1d1d1f] transition-[background-color,color,padding-left] duration-200 hover:bg-blue-50 hover:pl-5 hover:text-[#0071e3]">{link.label}</button>)}
            </div>
          </div>
        </div>
      </nav>
