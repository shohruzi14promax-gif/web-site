...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4 sm:px-6 sm:pt-5 lg:px-8">
      <nav style={navStyle} className="glass-nav mx-auto flex min-h-[68px] w-full max-w-6xl items-center justify-between rounded-[20px] px-4 py-2.5 transition-[background-color,border-color,box-shadow,backdrop-filter] duration-200 sm:px-5">
        ...
        <div ref={menuRef} className="relative lg:hidden">
          ...
          <div id="mobile-navigation" className={`glass-nav absolute right-0 top-[72px] w-[min(90vw,360px)] overflow-hidden rounded-[24px] transition-all duration-300 ${menuOpen ? 'visible translate-y-0 scale-100 opacity-100' : 'invisible -translate-y-2 scale-[.98] opacity-0'}`}>
            <div className="p-2.5">
              {menuLinks.map(link => (
