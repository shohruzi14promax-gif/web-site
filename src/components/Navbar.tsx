...
export default function Navbar() {
  ...
  const openSchoolCoin = () => {
    setMenuOpen(false);
    setMoreOpen(false);
    window.dispatchEvent(new CustomEvent('open-schoolcoin'));
  };
  ...
  const navButton = (link: NavLink) => {
    ...
    return (
      <button
        ...
        className={`group relative whitespace-nowrap rounded-xl px-3 py-2 text-[12px] font-semibold transition-all duration-300 active:scale-[.96] xl:text-[13px] ${active ? 'text-[#0071e3] dark:text-white' : 'text-slate-700 hover:text-[#0071e3] dark:text-slate-200 dark:hover:text-white'}`}
      >
    ...
    <div className="flex items-center rounded-2xl border border-slate-200/70 bg-slate-100/70 p-0.5 shadow-inner dark:border-white/10 dark:bg-white/5" aria-label={t('language')}>
      {(['uz', 'ru', 'en'] as Locale[]).map(item => (
        <button
          ...
          className={`rounded-xl px-2.5 py-1.5 text-[10px] font-extrabold uppercase transition-all duration-300 active:scale-95 ${locale === item ? 'bg-white text-[#0071e3] shadow-sm dark:bg-white dark:text-slate-900' : 'text-slate-500 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white'}`}
        >
          {item}
        </button>
      ))}
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-2 sm:px-3">
      <nav
        ref={menuRef}
        className={`mx-auto mt-2 flex max-w-7xl items-center gap-2 rounded-[22px] border px-3 py-2 transition-all duration-500 sm:px-4 ${scrolled ? 'border-slate-200/70 bg-white/85 shadow-[0_12px_40px_rgba(15,23,42,.12)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/85' : 'border-white/60 bg-white/65 shadow-[0_8px_32px_rgba(15,23,42,.06)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/70'}`}
      >
        <button type="button" onClick={() => navigate('#hero')} className="group flex shrink-0 items-center gap-2 rounded-xl px-1.5 py-1.5 active:scale-[.96]" aria-label={t('home')}>
          <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#0071e3]"><GraduationCap className="h-5 w-5 text-white" /></span>
        ...
        <div className="hidden min-w-0 flex-1 items-center justify-center gap-0.5 xl:flex">
          ...
          <div className="relative">
            ...
            {moreOpen && (
              <div className="absolute right-0 top-12 w-64 rounded-3xl border border-slate-200/70 bg-white/95 p-2 shadow-[0_20px_60px_rgba(15,23,42,.18)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/95">
                <p className="px-3 py-2 text-[10px] font-bold uppercase tracking-[.16em] text-slate-400">{t('digital')}</p>
                {extraLinks.map(link => <button key={link.href} type="button" onClick={() => navigate(link.href)} className="flex w-full rounded-2xl px-3 py-2.5 text-left text-sm font-semibold text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-white/10">{t(link.key)}</button>)}
                <div className="my-2 border-t border-slate-200 dark:border-white/10" />
                <button type="button" onClick={openAdmin} className="flex w-full rounded-2xl px-3 py-2.5 text-left text-sm font-bold text-slate-800 hover:bg-slate-100 dark:text-white dark:hover:bg-white/10">⚙️ {t('admin')}</button>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="ml-auto hidden items-center gap-1.5 lg:flex">
          {localeButtons}
          <button type="button" onClick={changeTheme} className="flex h-9 w-9 items-center justify-center rounded-2xl border border-slate-200/70 bg-slate-100/70 text-slate-700 dark:border-white/10 dark:bg-white/5 dark:text-white" aria-label={t('theme')} title={dark ? t('light') : t('dark')}>
            {dark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
          </button>
          <button type="button" onClick={openSchoolCoin} className="rounded-2xl border border-amber-200/70 bg-amber-50 px-3 py-2 text-[12px] font-extrabold text-amber-700 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:border-amber-400/20 dark:bg-amber-500/10 dark:text-amber-300">
            🪙 {t('schoolCoin')}
          </button>
        </div>

        <div className="relative ml-auto lg:hidden">
          <button type="button" onClick={() => setMenuOpen(value => !value)} className="flex h-10 w-10 items-center justify-center rounded-2xl border border-slate-200/70 bg-white/75 text-slate-800 shadow-sm dark:border-white/10 dark:bg-white/10 dark:text-white" aria-label={menuOpen ? t('closeMenu') : t('openMenu')} aria-expanded={menuOpen}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-12 max-h-[80vh] w-[min(94vw,390px)] overflow-y-auto rounded-[28px] border border-slate-200/70 bg-white/95 p-3 shadow-[0_24px_70px_rgba(15,23,42,.2)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/95">
              ...
              <div className="flex items-center justify-between gap-2 px-2 py-2">
                {localeButtons}
                <button type="button" onClick={changeTheme} className="flex h-9 w-9 items-center justify-center rounded-2xl border border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/5" aria-label={t('theme')}>{dark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</button>
              </div>
              <button type="button" onClick={openSchoolCoin} className="w-full rounded-2xl bg-amber-50 px-4 py-3 text-left text-sm font-extrabold text-amber-700 hover:bg-amber-100 dark:bg-amber-500/10 dark:text-amber-300">🪙 {t('schoolCoin')}</button>
              <button type="button" onClick={openAdmin} className="mt-1 w-full rounded-2xl px-4 py-3 text-left text-sm font-bold text-slate-800 hover:bg-slate-100 dark:text-white dark:hover:bg-white/10">⚙️ {t('admin')}</button>
            </div>
          )}
        </div>
      </nav>
