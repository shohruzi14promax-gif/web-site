...
function StatCard({ stat, visible }: { stat: (typeof stats)[number]; visible: boolean }) {
  ...
  return (
    <div className="border-l border-slate-200 px-4 py-3 text-center first:border-l-0 sm:px-5">
  ...
  return (
    <section id="hero" className="relative overflow-hidden bg-slate-950 pt-28 text-white sm:pt-32">
      <div className="absolute inset-0">
        <img src="/images/maktab-bg.JPG" alt={schoolInfo.shortName} className="h-full w-full object-cover opacity-35" />
      ...
      <div ref={ref} className="relative mx-auto grid min-h-[680px] max-w-7xl items-center gap-12 px-5 py-20 sm:px-8 md:grid-cols-[1.15fr_.85fr] md:px-10 lg:px-12 lg:py-28">
        <div className={`max-w-3xl transition-all duration-700 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
          <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-xs font-bold uppercase tracking-[.16em] text-blue-200">
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            <button type="button" onClick={() => scrollTo('#about')} className="school-button bg-blue-600 text-white">
              {t('schoolAbout')}<ArrowRight className="ml-2 h-4 w-4" />
            </button>
            <button type="button" onClick={() => window.dispatchEvent(new CustomEvent('open-schoolcoin'))} className="school-button border border-white/20 bg-white/10 text-white">
        ...
        <div className="hidden md:block">
          <div className="ml-auto max-w-md rounded-[24px] border border-white/15 bg-white/10 p-2 shadow-2xl backdrop-blur-md">
            <div className="rounded-[18px] border border-white/10 bg-slate-950/35 p-7">
              <div className="flex items-center justify-between border-b border-white/10 pb-5">
                <div><p className="text-xs font-bold uppercase tracking-[.16em] text-blue-200">Digital School</p><p className="mt-1 text-lg font-bold">{t('schoolCoin')}</p></div>
                <Coins className="h-5 w-5 text-amber-300" />
              </div>
              <div className="grid grid-cols-2 gap-3 pt-5">
                {[t('academic'), t('innovation'), t('life'), t('schoolCoin')].map(item => <div key={item} className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-slate-200">{item}</div>)}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="relative mx-auto max-w-7xl px-5 pb-8 sm:px-8 md:px-10 lg:px-12">
        <div className="grid grid-cols-2 overflow-hidden rounded-[20px] border border-slate-200/15 bg-white p-2 text-slate-950 shadow-2xl sm:grid-cols-3 lg:grid-cols-6">
          {stats.map(stat => <StatCard key={stat.label} stat={stat} visible={isVisible} />)}
        </div>
      </div>

      <button type="button" onClick={() => scrollTo('#about')} className="absolute bottom-4 left-1/2 hidden -translate-x-1/2 items-center gap-2 text-[10px] font-bold uppercase tracking-[.18em] text-slate-300 sm:flex" aria-label={t('explore')}>
        {t('explore')}<ChevronDown className="h-4 w-4" />
      </button>
    </section>
  );
}
