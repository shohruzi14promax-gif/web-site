    ...
    <section id="hero" className="relative isolate overflow-hidden bg-slate-950 pt-28 text-white sm:pt-32">
      ...
      <div ref={ref} className="relative mx-auto grid min-h-[680px] max-w-7xl items-center gap-12 px-5 py-20 sm:px-8 md:grid-cols-[1.15fr_.85fr] md:px-10 lg:px-12 lg:py-28">
        <div className="max-w-3xl">
          <div className={`hero-reveal hero-reveal-1 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-bold uppercase tracking-[.16em] text-blue-100 shadow-lg shadow-black/10 backdrop-blur-xl ${reveal}`}>
            <GraduationCap className="h-4 w-4" />1-IMI Jizzax
          </div>
          <h1 className={`hero-reveal hero-reveal-2 mt-7 text-5xl font-black leading-[.98] tracking-[-.045em] drop-shadow-[0_12px_35px_rgba(0,0,0,.25)] sm:text-7xl lg:text-[5.4rem] ${reveal}`}>
            {t('learn')}.<br />{t('create')}.<br /><span className="text-blue-400">{t('lead')}.</span>
          </h1>
          <p className={`hero-reveal hero-reveal-3 mt-7 max-w-2xl text-base leading-7 text-slate-200/95 sm:text-lg lg:text-xl ${reveal}`}>{t('heroDescription')}</p>
          <div className={`hero-reveal hero-reveal-4 mt-9 flex flex-col gap-3 sm:flex-row ${reveal}`}>
      ...
      <div className="relative mx-auto max-w-7xl px-5 pb-8 sm:px-8 md:px-10 lg:px-12">
        <div className="hero-stats grid grid-cols-2 overflow-hidden rounded-[20px] p-2 text-slate-950 sm:grid-cols-3 lg:grid-cols-6">
