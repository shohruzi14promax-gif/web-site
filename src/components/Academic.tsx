        ...
        {tab === 'subjects' && selected && (
          <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
            ...
            <div className="rounded-[28px] border border-white/70 bg-white/80 p-7 shadow-xl backdrop-blur-xl sm:p-9">
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-3"><div className="rounded-2xl bg-[#f5f5f7] p-5"><div className="text-xs uppercase tracking-wide text-[#6e6e73]">Yo'nalish</div><div className="mt-2 font-semibold text-[#1d1d1f]">Nazariya va amaliyot</div></div><div className="rounded-2xl bg-[#f5f5f7] p-5"><div className="text-xs uppercase tracking-wide text-[#6e6e73]">Dastur</div><div className="mt-2 font-semibold text-[#1d1d1f]">Chuqurlashtirilgan ta'lim</div></div><div className="rounded-2xl bg-[#f5f5f7] p-5"><div className="text-xs uppercase tracking-wide text-[#6e6e73]">Maqsad</div><div className="mt-2 font-semibold text-[#1d1d1f]">Olimpiada va OTMga tayyorgarlik</div></div></div>
              <div className="mt-8 border-t border-black/5 pt-7"><h4 className="text-lg font-semibold text-[#1d1d1f]">{selected.name} bo'yicha imkoniyatlar</h4><div className="mt-4 grid gap-3 sm:grid-cols-2">{['Amaliy mashg‘ulotlar', 'Fan olimpiadalariga tayyorgarlik', 'Loyiha va tadqiqot ishlari', 'Qo‘shimcha o‘quv materiallari'].map((item) => <div key={item} className="rounded-xl bg-[#f5f5f7] px-4 py-3 text-sm text-[#6e6e73]">✓ {item}</div>)}</div></div>
              <div className="mt-8 border-t border-black/5 pt-7"><h4 className="text-lg font-semibold text-[#1d1d1f]">Shu fan o'qituvchilari ({subjectTeachers.length})</h4>{subjectTeachers.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2">{subjectTeachers.map((teacher, i) => <div key={`${teacher.name}-${i}`} className="rounded-2xl border border-black/5 bg-white p-4"><div className="font-semibold text-[#1d1d1f]">{teacher.name}</div><div className="mt-1 text-xs text-[#6e6e73]">{teacher.classes} · {teacher.category}</div></div>)}</div> : <p className="mt-4 text-sm text-[#6e6e73]">Bu fan bo‘yicha o‘qituvchilar topilmadi.</p>}</div>
            </div>
          </div>
        )}
            ...
            <>
              <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
                {visibleTeachers.map((teacher, i) => (
                  <div key={`${teacher.name}-${i}`} className="apple-card !p-6">
                    <div className="mb-5 h-40 overflow-hidden rounded-2xl bg-slate-100">{teacher.image ? <img src={teacher.image} alt={teacher.name} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center"><Users className="h-12 w-12 text-[#0071e3]/50" /></div>}</div>
                    <h3 className="font-semibold text-[#1d1d1f]">{teacher.name}</h3><p className="mt-1 text-sm font-medium text-[#0071e3]">{teacher.subject}</p>
                    <div className="mt-5 space-y-2 border-t border-black/5 pt-4 text-xs"><div className="flex justify-between"><span className="text-[#6e6e73]">Sinflar</span><span>{teacher.classes}</span></div><div className="flex justify-between"><span className="text-[#6e6e73]">Staj</span><span>{teacher.experience}</span></div><div className="flex justify-between"><span className="text-[#6e6e73]">Toifa</span><span>{teacher.category}</span></div></div>
                  </div>
                ))}
              </div>
