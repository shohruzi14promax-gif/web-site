...
const iconMap: Record<string, typeof Calculator> = { Calculator, Atom, Code2, FlaskConical, Dna, Languages, Landmark, BookOpen };
...
type AcademicTeacher = { name: string; subject: string; classes: string; experience: string; category: string; image: string };
type GpaRow = { name: string; class?: string; gpa: number | string; rank?: number; achievements?: string };

const normalizeTeachers = (list: unknown[]): AcademicTeacher[] => list.map((value) => {
  const teacher = (value && typeof value === 'object' ? value : {}) as Record<string, unknown>;
  return {
    name: String(teacher.name || teacher.fullName || "O'qituvchi"),
    subject: String(teacher.subject || teacher.role || teacher.fan || "Fan o'qituvchisi"),
    classes: String(teacher.classes || teacher.sinf || 'Aniqlanmoqda'),
    experience: String(teacher.experience || teacher.staj || 'Aniqlanmoqda'),
    category: String(teacher.category || teacher.toifa || 'Aniqlanmoqda'),
    image: String(teacher.image || teacher.avatar || teacher.photo || ''),
  };
});
...
export default function Academic() {
  ...
  const [teachersList, setTeachersList] = useState<AcademicTeacher[]>(normalizeTeachers(defaultTeachers));
  const [gpaList, setGpaList] = useState<GpaRow[]>(defaultGpa as GpaRow[]);
  ...
  useEffect(() => {
    ...
    const loadAcademic = async () => {
      try {
        const [teacherData, gpaData] = await Promise.all([
          getSiteData('teachers', [] as unknown[]),
          getSiteData('gpaList', [] as unknown[]),
        ]);
        if (!alive) return;
        if (Array.isArray(teacherData) && teacherData.length) setTeachersList(normalizeTeachers(teacherData));
        if (Array.isArray(gpaData) && gpaData.length) setGpaList(gpaData as GpaRow[]);
      } catch {
        // Keep the bundled fallback data when the database is unavailable.
      }
    };
    void loadAcademic();
    window.addEventListener('admin_data_updated', loadAcademic);
    return () => { alive = false; window.removeEventListener('admin_data_updated', loadAcademic); };
  }, []);

  const selected = useMemo(() => subjects.find((subject) => subject.name === selectedSubject) || subjects[0], [selectedSubject]);
  const subjectTeachers = useMemo(() => teachersList.filter((teacher) => {
    if (!selected) return false;
    const value = teacher.subject.toLowerCase().trim();
    const target = selected.name.toLowerCase().trim();
    return value.includes(target) || target.includes(value);
  }), [selected, teachersList]);
  ...
  const tabs = [
    ...
    { key: 'gpa' as Tab, label: 'GPA Reyting', icon: TrendingUp },
  ];
