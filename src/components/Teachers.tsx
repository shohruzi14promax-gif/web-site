  ...
  const loadTeachers = async () => {
    ...
    try {
      if (supabaseConfigured) {
        const siteData = await getSiteData<Teacher[]>('teachers', []);
        if (siteData.length > 0) {
          setTeachers(siteData);
          localStorage.setItem('teachers', JSON.stringify(siteData));
          return;
        }
        ...
        if (!error && tableData?.length) {
          setTeachers(tableData as Teacher[]);
          localStorage.setItem('teachers', JSON.stringify(tableData));
          return;
        }
      }
    ...
    <section id="teachers" className="apple-section relative overflow-hidden py-20">
      ...
      <div className="apple-container mx-auto max-w-7xl px-4">
        ...
        <div className="mb-10 flex flex-wrap justify-center gap-2">
          {subjects.map(item => (
            <button key={item} onClick={() => { setSubject(item); setShowAll(false); }} className={`rounded-full px-4 py-2 text-sm font-semibold transition-all ${subject === item ? 'bg-[#0071e3] text-white shadow-lg' : 'border border-slate-200 bg-white/70 text-slate-600 hover:bg-white'}`}>
              {item}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" aria-label="O'qituvchilar yuklanmoqda">
            {Array.from({ length: 8 }).map((_, index) => <div key={index} className="h-64 animate-pulse rounded-[28px] border border-white/80 bg-white/60" />)}
          </div>
        ) : (
          <div ref={ref} className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {visibleTeachers.map((teacher, index) => (
              <article key={teacher.id} className={`group relative overflow-hidden rounded-[28px] border border-white/80 bg-white/70 p-5 shadow-[0_10px_40px_rgba(15,23,42,0.06)] backdrop-blur-2xl transition-all duration-700 hover:-translate-y-2 hover:shadow-[0_25px_60px_rgba(15,23,42,0.12)] ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: `${Math.min(index, 8) * 70}ms` }}>
                <div className="mb-5 flex items-center gap-4">
                  <div className="h-20 w-20 shrink-0 overflow-hidden rounded-[20px] border border-white bg-slate-100 shadow-md">
                    <img src={teacher.image || '/images/shohruz.jpg'} alt={teacher.name} className="h-full w-full object-cover" loading="lazy" />
                  </div>
                ...
                <div className="mt-5 grid grid-cols-2 gap-2 border-t border-slate-200/70 pt-4">
                  <div className="rounded-2xl bg-slate-50/80 p-3"><div className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-400"><BookOpen className="h-3.5 w-3.5" /> Tajriba</div><div className="mt-1 text-sm font-bold text-slate-700">{teacher.experience || '—'}</div></div>
                  <div className="rounded-2xl bg-slate-50/80 p-3"><div className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-400"><Users className="h-3.5 w-3.5" /> Sinflar</div><div className="mt-1 truncate text-sm font-bold text-slate-700">{teacher.classes || 'Aniqlanmoqda'}</div></div>
                </div>
        ...
        {!loading && filteredTeachers.length > 8 && (
          <div className="mt-10 flex justify-center">
            <button onClick={() => setShowAll(value => !value)} className="inline-flex items-center gap-2 rounded-full bg-[#0071e3] px-6 py-3 text-sm font-bold text-white shadow-lg shadow-blue-500/20 transition-all hover:-translate-y-0.5 hover:bg-blue-700">
