...
const getTashkentTimestamp = () => new Intl.DateTimeFormat('uz-UZ', { timeZone: 'Asia/Tashkent', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false }).format(new Date());
...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); const text = message.trim(); if (!text || sending) return; setSending(true);
    ...
    if (error) { console.error('Murojaatni yuborishda xatolik:', error); setSending(false); return; }
    console.info('Murojaat yuborildi:', getTashkentTimestamp()); setName(''); setMessage(''); setSubmitted(true); setSending(false); window.setTimeout(() => setSubmitted(false), 4000);
  };
  return (
